	...
	<main class="flex-1 px-4 py-6">
		...
		{:else if !player}
			<div class="py-20 text-center">
				...
				<button on:click={() => goto('/')} class="mt-4 text-blue-700 underline"
		...
		{:else}
			<form on:submit|preventDefault={handleSubmit} class="space-y-5">
				...
				<!-- Number -->
				<div>
					<label for="number" class="mb-1.5 block text-sm font-medium text-gray-700"
				...
				<div>
					<p class="mb-2 text-sm font-medium text-gray-700">
						Position <span class="text-gray-400">(optional)</span>
					</p>
					<div class="grid grid-cols-4 gap-2">
						{#each POSITIONS as pos}
							<button
								...
								class="rounded-xl border-2 py-2.5 text-sm font-semibold transition-colors {position ===
								pos.value
			...
			<div class="mt-8 border-t border-gray-200 pt-6">
				...
				{#if confirmDelete && !deleting}
					<button
						...
						class="mt-2 w-full py-2 text-sm text-gray-500"
					>
						Cancel
					</button>
				{/if}
